...
class AtlanticPassAPCHeatingZone(OverkizEntity, ClimateEntity):
    ...
    def __init__(
        ...
        self.temperature_device = self.executor.linked_device(
            (self.device.identifier.subsystem_id or 0) + 1
        )
    ...
    def hvac_mode(self) -> HVACMode:
        ...
        return OVERKIZ_TO_HVAC_MODE[
            cast(
                str, self.device.get_state_value(OverkizState.IO_PASS_APC_HEATING_MODE)
            )
        ]
    ...
    def current_heating_profile(self) -> str:
        ...
        return cast(
            str,
            self.device.get_state_value(OverkizState.IO_PASS_APC_HEATING_PROFILE),
        )
